...
def get_style(tag):
    ...
    for key in ("fill", "stroke"):
        if key in style:
            style[key] = (
                None if style[key] == "none" else palettetools.parse_color(style[key])
            )
    return style


def get_transform(tag):
    if not tag.has_attr("transform"):
        return (0, 0)
    tokens = re.split("\(|,|\)", tag["transform"])
    if tokens[0] != "translate":
        raise Exception(
            "Don't understand {}.transform: '{}'".format(tag.name, tag["transform"])
        )
    else:
    ...
    for char in path:
        if char in digit_exp:
            ...
            if entity:
                ...
                entity = ""
            if len(entities) > 1:
                yield entities[0], [float(x) for x in entities[1:]]
            elif len(entities) == 1:
                yield entities[0], None
            entities = []
...
class Path(Polyline):
    def __init__(self, tag, offset):
        ...
        for cmd, args in parse_path(tag["d"]):
            ...
            if cmd == "M":
                ...
                if len(args) > 2:
                    self.polyline_abs(args[2:])
            elif cmd == "m":
                if not self.lines:
                    self.moveto_abs(args[:2])
                else:
                    self.moveto_rel(args[:2])
                if len(args) > 2:
                    self.polyline_rel(args[2:])
            elif cmd == "Z" or cmd == "z":
                self.closepath()
            elif cmd == "L":
                ...
                raise Exception(
                    "Don't understand command '{}' of path {}".format(cmd, tag["id"])
                )
    ...
    def hline_abs(self, args):
        pts = []
        for x in args:
            pts += [x + self.offset[0], self.cursor[1]]
    ...
    def vline_abs(self, args):
        pts = []
        for y in args:
            pts += [self.cursor[0], y + self.offset[1]]
        self.polyline_abs(pts)

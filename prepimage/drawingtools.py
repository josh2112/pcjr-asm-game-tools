...
def is_colorable(x, y, img):
    return img.getpixel((x, y)) == 15
...
def flood_fill(pt, color, img):
    ...
    while pixels:
        x, y = pixels.pop()
        while x >= 0 and is_colorable(x, y, img):
            x -= 1
        x += 1
        ...
        while x < img.size[0] and is_colorable(x, y, img):
            ...
            if not span_up and y > 0 and is_colorable(x, y - 1, img):
                pixels.append((x, y - 1))
                span_up = True
            elif span_up and (y == 0 or not is_colorable(x, y - 1, img)):
                span_up = False
            ...
            if not span_down and y < img.size[1] - 1 and is_colorable(x, y + 1, img):
                ...
                y == img.size[1] - 1 or not is_colorable(x, y + 1, img)
            ):
                span_down = False
...
class SVGSimplifier:
    def __init__(self, img, callback_update_image=None):
        self.img = img
        self.callback_update_image = callback_update_image
        self.cmds = []
        self.plotter = Plotter()

    def prepare(self, pt):
        return clamp(round(pt[0] / 2), self.img.size[0] - 1), clamp(
            round(pt[1]), self.img.size[1] - 1
        )
    ...
    def render(self, figure):
        ...
        for pair in figure.lines:
                ...
                if is_colorable(pt[0], pt[1], self.img) or not fill_points:
                    ...
                    if self.plotter.plotting:
                        self.cmds.append(
                            (
                                Commands.LINETO,
                                self.plotter.point[0],
                                self.plotter.point[1],
                            )
                        )
            ...
            if self.plotter.plotting:
                self.cmds.append(
                    (Commands.LINETO, self.plotter.point[0], self.plotter.point[1])
                )
                self.plotter.cursor = self.plotter.point
            self.plotter.point = pt
    ...
    def clean(self):
        ...
        for i in range(num_commands - 1, 1, -1):
            if self.cmds[i] == self.cmds[i - 1]:
                ...
                self.cmds[i] == self.cmds[i - 1] == self.cmds[i - 2] == Commands.LINETO
            ):
                angles = [
                    math.atan2(
                        self.cmds[j][2] - self.cmds[j - 1][2],
                        self.cmds[j][1] - self.cmds[j - 1][1],
                    )
                    for j in (i, i - 1)
                ]
...
class SimpleSVGRenderer:
    ...
    def render(self, cmds):
        ...
        print(
            "{} command(s) (upper bound {} bytes)".format(
                len(cmds), sum(len(c) for c in cmds)
            )
        )
